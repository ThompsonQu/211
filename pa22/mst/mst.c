...
int main(int argc, char *argv[]) {
		...
		for (graphNode_t source = 0; source < graphNodeCount; source++) {
			if (parents[source] != -1) { // if already visited
				...
				while (nextNodes != NULL) {
					...
					double nextNodeWeight = nextNodes->weight;

					if (parents[nextNode] != -1) {
						nextNodes = nextNodes->next;
					} else {
						//the neighbor has not been reached.
						if (nextNodeWeight < minWeight) {
							minWeight = nextNodeWeight;
							minSource = source;
							minDest = nextNode;
						}
						nextNodes = nextNodes->next;
	...
	/* ... */
	size_t currNode = 0;
	while (currNode < graphNodeCount) {
		graphNode_t prevNode = parents[currNode];
		if (prevNode != -1 && prevNode != currNode) {
